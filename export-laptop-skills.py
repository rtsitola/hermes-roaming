#!/usr/bin/env python3
"""
LAPTOP SCRIPT — export-laptop-skills.py
Run at END of laptop session (or periodically).
...
MAX_FILE_SIZE = 100 * 1024  # 100 KB — skip large schemas/templates
...
def find_skills():
    """Walk SKILLS_DIR and find all skill directories (containing SKILL.md)."""
    skills = []
...
def collect_skill_files(skill_root):
    """Collect all files in a skill directory, skipping large/binary files."""
    files = {}
...
def export():
    ...
    for skill_root, rel_dir in skills:
        ...
        # Collect supporting files (excluding SKILL.md itself — we handle that separately)
    ...
    export_path = f"{SHARED}/laptop-skills.json"
    ...
    return f"Exported {len(export_data)} skills ({total_files} files, {size_kb:.0f} KB) to laptop-skills.json"
